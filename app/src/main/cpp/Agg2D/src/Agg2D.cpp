...
void Agg2D::polygon(double* xy, int numPoints)
{
    ...
    */
	...
	m_path.remove_all();
	m_path.move_to(xy[0], xy[1]);
	xy +=2;
	for(int i = 1; i < numPoints; i++)
	{
		m_path.line_to(xy[0], xy[1]);
		xy +=2;
	}
	m_path.close_polygon();
	drawPath(FillAndStroke);		
...
void Agg2D::polyline(double* xy, int numPoints)
{
    ...
    m_path.remove_all();
    //m_path.add_poly(xy, numPoints);
	m_path.concat_poly(xy,numPoints,false); // JME, AF
    drawPath(StrokeOnly);
	*/
...
//------------------------------------------------------------------------
void   Agg2D::fontProperties(double height, double angle)
{
	m_fontHeight = height;
	m_textAngle = angle;
#ifdef AGG2D_USE_FREETYPE	
...
double Agg2D::textWidth(const char* str)
{
    ...
    while(*str)
    {
        const agg::glyph_cache* glyph = m_fontCacheManager.glyph((unsigned char)*str);
        if(glyph)
        {
...
//------------------------------------------------------------------------
double Agg2D::textWidth(const short* str)
{
    double x = 0;
    double y = 0;
    bool first = true;
    while(*str)
    {
        const agg::glyph_cache* glyph = m_fontCacheManager.glyph((unsigned short)*str);
        if(glyph)
        {
            if(!first) m_fontCacheManager.add_kerning(&x, &y);
            x += glyph->advance_x;
...
void Agg2D::text(double x, double y, const char* str, bool roundOff, double ddx, double ddy)
{
   ...
   agg::trans_affine  mtx;
    ...
    for (i = 0; str[i]; i++)
    {
        glyph = m_fontCacheManager.glyph((unsigned char)str[i]);
        if(glyph)
        {
...
//------------------------------------------------------------------------
void Agg2D::text(double x, double y, const short* str, bool roundOff, double ddx, double ddy)
{
   double dx = 0.0;
   double dy = 0.0;
   ...
   agg::trans_affine  mtx;
    ...
    for (i = 0; str[i]; i++)
    {
        glyph = m_fontCacheManager.glyph((unsigned short)str[i]);
        if(glyph)
        {
            if(i) m_fontCacheManager.add_kerning(&x, &y);
            m_fontCacheManager.init_embedded_adaptors(glyph, start_x, start_y);
...
public:
	...
	template<class BaseRenderer, class Interpolator>
	static void renderImagePerspective(Agg2D& gr, const Agg2D::Image& img,
		BaseRenderer& renBase, Interpolator& interpolator)
	{
		//! JME - have not quite figured which part of this is not const-correct
		// hence the cast.
...
void Agg2D::renderImagePerspective(const Image& img, int x1, int y1, int x2, int y2, 
													 int x3, int y3, int x4, int y4)
{
	...
	m_rasterizer.clip_box(0, 0, m_rbuf.width(), m_rbuf.height());
	m_rasterizer.reset();
	m_rasterizer.move_to_d(x1, y1);
	m_rasterizer.line_to_d(x2, y2);
	m_rasterizer.line_to_d(x3, y3);
	m_rasterizer.line_to_d(x4, y4);

	agg::trans_perspective tr(poly, 0, 0, img.width(), img.height());
	...
	if(tr.is_valid())
	{
		...
		{
			// JME audit -
			Agg2DRenderer::renderImagePerspective(*this,img, m_renBasePre, interpolator);
		}
		else
		{
			Agg2DRenderer::renderImagePerspective(*this,img, m_renBaseCompPre, interpolator);
		}
	}	
