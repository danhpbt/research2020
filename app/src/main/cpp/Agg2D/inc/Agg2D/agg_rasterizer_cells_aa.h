...
namespace agg
{
    ...
    //------------------------------------------------------------------------
    template <class Cell> 
    static AGG_INLINE void swap_cells(Cell* a, Cell* b)
    {
        Cell temp = *a;
        *a = *b;
        *b = temp;
    ...
    void qsort_cells(Cell** start, unsigned num)
    {
        ...
        for (;;)
        {
            ...
            if(len > qsort_threshold)
            {
                ...
                pivot = base + len / 2;
                //pbtdanh swap_cells(base, pivot);
                temp = *base; *base = *pivot; *pivot = temp;
                ...
                if((*base)->x < (*i)->x)
                {
                    //pbtdanh swap_cells(base, i);
                	temp = *base; *base = *i; *i = temp;
                }

                if((*j)->x < (*base)->x)
                {
                    //pbtdanh swap_cells(base, j);
                	temp = *base; *base = *j; *j = temp;
                }
            ...
            else
            {
                ...
                for(; i < limit; j = i, i++)
                {
                    for(; j[1]->x < (*j)->x; j--)
                    {
                        //pbtdanh swap_cells(j + 1, j);
                    	temp = *(j + 1); *(j + 1) = *j; *j = temp;
