...
public:
    ...
    void   fontProperties(double height, double angle);
    ...
    double textWidth(const short* str);
    void   text(double x, double y, const short* str, bool roundOff=false, double dx=0.0, double dy=0.0);
#endif
    // Path commands
    //-----------------------
    void resetPath();
...
public: 
	//pbtdanh
	void renderImagePerspective(const Image& img, int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4);
private:
